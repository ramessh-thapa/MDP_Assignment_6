...
class HistoricalEventDialogActivity : AppCompatActivity() {
    private lateinit var parentContext: FragmentManager
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_historical_event_dialog)

    }

    fun showDialog(parentFragmentManager: FragmentManager, context: Context, callback: (HistoricalActivity) -> Unit) {
        this.parentContext = parentFragmentManager
        val dialog = Dialog(context)
        dialog.requestWindowFeature(Window.FEATURE_NO_TITLE)
        dialog.setCancelable(false)
        dialog.setContentView(R.layout.activity_historical_event_dialog)
        ...
        addButton.setOnClickListener {
            ...
            if (name.isNullOrBlank() || date.isNullOrBlank() || description.isNullOrBlank()) {
                ...
                Toast.makeText(context, "Historical Event added successfully.", Toast.LENGTH_SHORT).show()
                callback(
                    HistoricalActivity(name, date, description)
                );
                dialog.dismiss()
            }
        }
